...
namespace WebServer
{
    ...
    class WebServer
    {
        ...
        private IScriptProcessor _scriptProcessor, _templateProcessor;
        ...
        static void Main(string[] args)
        {
            ...
            string webRoot = args.Count() > 1 ? args[1] : @"..\..\..\wwwroot";

            string path = args.Count() > 2 ? args[2] : @"index.html";

            /* create an instance of the web server and start listening for requests */
            new WebServer(port, webRoot, path);
        }

        public WebServer(int port, string root, string path)
        {
            /* this script processor instance will be used to process files of type 
             * csscript */
            _scriptProcessor = new CscriptProcessor();

            /*another instance of a IScriptProcessor to handle files of
             * type csweb */
        ...
        public void AcceptRequest(Socket socket)
        {
            if (socket.Connected)
            {
                ...
                /* change this to support a default document (i.e. index.html) as
